...
ui <- fluidPage(
  ...
  tags$style(HTML("
    ...
    /* Style for the first row of numbers in grid 1 */
    .count-row {
    margin-left: 2px;  /* Décalage pour correspondre aux autres cases de la grille */
    }
...
server <- function(input, output, session) {
  ...
  output$grid1RowNumbers <- renderUI({
    ...
    div(
      ...
      lapply(1:min(gridSize, ncol(values$grid1)), function(col) {  # Commencez à partir de la deuxième colonne
        div(
          id = paste0("countRowGrid1_", col),
          class = "count-row",  # Ajoutez la classe count-row
          style = if (gridSize == 8) {
